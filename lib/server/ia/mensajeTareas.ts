// Pipeline de IA para gestión de tareas/notas/horario por lenguaje natural.
//
// EXTRAÍDO de app/api/ai/tareas/route.ts en el Sprint 2/3 (menús): el canal
// de WhatsApp necesita exactamente esto mismo, y la única forma de no
// duplicar ~450 líneas de carga de contexto y ejecución de operaciones era
// sacarlas del Route Handler a un módulo que ambos importen. Mismo patrón y
// mismo motivo que `crearTareaServidor` (lib/server/tareas.ts) y
// `crearBloque`/`actualizarBloque` (lib/server/horario.ts).
//
// El comportamiento NO cambia: es un movimiento de código. Todos los
// comentarios que explican cada bug ya corregido (el tercer argumento que
// anulaba ContextEngine, la duración del bloque al mover una hora sola, el
// IDOR de los adjuntos) viajan con el código a su nuevo sitio.
//
// El Route Handler conserva lo que le toca a un borde HTTP: autenticar,
// parsear el body, verificar que los adjuntos son del usuario, y serializar.
...
export type EntradaMensajeTareas = {
  /** Texto del usuario. Puede ir vacío si el mensaje son solo adjuntos. */
  texto: string
  /** Rutas de Storage ya verificadas como del usuario por quien llama. */
  rutasAdjuntos?: string[]
  /** Historial de la sesión, si el llamador lo mantiene (el overlay de /ai sí; WhatsApp no). */
  historial?: unknown
}

/**
 * Corre el turno completo: carga contexto, llama a TaskManagementAgent, y
 * ejecuta del lado del servidor las operaciones de NOTA y de BLOQUE DE
 * HORARIO (que nunca viajan al cliente). Las operaciones de TAREA quedan en
 * `result.output.operaciones` sin aplicar — el overlay de /ai las muestra
 * con aplicar/deshacer; otros llamadores (WhatsApp) las aplican ellos.
 */
export async function procesarMensajeTareas(
  userId: string,
  entrada: EntradaMensajeTareas
): Promise<{ ok: true; result: Awaited<ReturnType<typeof aiOrchestrator.execute<TaskManagementAgentOutput>>> } | { ok: false; error: string; status: number }> {
  const rutasAdjuntos = entrada.rutasAdjuntos ?? []
  const text = entrada.texto
  const historial = entrada.historial

  const cargaAdjuntos = await cargarAdjuntos(rutasAdjuntos)
  if (!cargaAdjuntos.ok) return { ok: false, error: cargaAdjuntos.error, status: 404 }
