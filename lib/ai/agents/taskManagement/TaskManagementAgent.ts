...
function construirInstruccionSistema(fechaISO: string, tareasExistentes: TareaContexto[], hayHistorial: boolean, hayAdjunto: boolean): string {
  return [
    ...
    'Primero decide tipoRespuesta: "operaciones" si el texto pide crear, modificar o borrar algo relacionado a tareas (aunque no logres resolverlo del todo); "conversacional" si el texto no tiene relación con gestionar tareas (saludo, pregunta, charla) — en ese caso "mensaje" lleva una respuesta breve y natural, aclarando que tu función es gestionar tareas académicas, y "operaciones" va vacío.',
...
//
// Lectura de tareas existentes: NO pasa por ContextEngine (su build() lanza
// a propósito para cualquier scope hasta que la Fase 2/Sprint 9 lo
// implemente de verdad — ver lib/ai/context/ContextEngine.ts) — es una
// consulta puntual que hace el Route Handler (app/api/ai/tareas/route.ts)
// antes de llamar a aiOrchestrator.execute(), pasada como
// request.metadata.tareasExistentes. Documentado acá porque es la decisión
// explícita que pedía el plan cuando ContextEngine todavía no cubre esto.
class TaskManagementAgentImpl implements AIAgent<TaskManagementAgentOutput> {
  readonly definition = definition

  async run(request: AIRequest, _context: AIContext, provider: AIProvider): Promise<AgentResult<TaskManagementAgentOutput>> {
    ...
    const metadata: StructuredProviderMetadata = {
      systemInstruction: construirInstruccionSistema(hoyEnZona(new Date(), ZONA_HORARIA_POR_DEFECTO), tareasExistentes, historial.length > 0, adjuntos.length > 0),
      outputSchema: TASK_MANAGEMENT_OUTPUT_SCHEMA,
